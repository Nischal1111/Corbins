  ...
  return (
    <main className={`relative w-full lg:mt-20 mt-8 pb-8 flex lg:flex-row flex-col gap-4 items-center justify-between  ${mainFont.className}`}>
      ...
      <motion.section initial="hidden"
                        ...
                        variants={fadeIn("up", "tween", 0.3, .8)} id='contact-form' className='relative flex flex-col items-center gap-12 lg:w-1/2 w-full'>
        ...
        <form  className='flex flex-col gap-16 w-full lg:px-16 px-10 lg:p-0 p-4'>
                  ...
                  <button onClick={()=>"mailto"} className='w-fit custom-button bg-transparent border border-white text-white rounded-sm py-2 px-6 -mt-4 relative tracking-widest text-base hover:bg-white hover:text-black duration-500' type='submit'>{loading ? "Sending..." :"Send"} </button>
              </form>
